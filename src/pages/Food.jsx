...
const Food = ({ closeModal }) => {
  const { id } = useParams();
  return (
    <motion.div
      className="fixed inset-0 z-10 overflow-y-auto bg-gray-900 bg-opacity-70 flex items-center py-12 justify-center"
      initial={{ width: 0 }}
      animate={{ width: "100%" }}
      ...
      <div className="relative rounded-lg text-left overflow-hidden transform transition-all w-[50%] flex flex-col items-center">
        ...
        <div className="px-4">
          ...
          <div className="container-culture h-full">
            <img src={food} className="rounded-tl-3xl w-[44%] 2xl:w-[50%]" />
          ...
          <div>
            <a>
              <div className="button-culture" onClick={closeModal}>
                ...
                <div>
                  <FaAnglesRight className="text-white title-profile" />
                </div>
              </div>
            </a>
          </div>
